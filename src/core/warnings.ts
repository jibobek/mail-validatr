...
export function addCustomWarningRule(
  rule: (email: string) => Warning | null
): void {
...
export function getWarnings(email: string): Warning[] {
  ...
  if (
    /^[a-zA-Z0-9\-]+\.[a-zA-Z]{1,2}$/.test(email) ||
  ...
  if (domainPart === 'localhost') {
    warnings.push({
      ...
      message:
        "Domain is 'localhost' – typically only used in testing environments.",
    })
  }
